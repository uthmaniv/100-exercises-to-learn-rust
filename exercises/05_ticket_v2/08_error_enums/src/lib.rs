...
impl Display for TicketNewError {
    fn fmt(&self, f: &mut Formatter) -> FmtResult {
        match self {
            TitleError(msg) => write!(f, "{}", msg),
            DescriptionError(msg) => write!(f, "{}", msg),
        }
    }
}
...
fn easy_ticket(title: String, mut description: String, status: Status) -> Ticket {
     if title.is_empty() {
            panic!("{}",TitleError("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            panic!("{}",TitleError("Title cannot be longer than 50 bytes".to_string()));
        }
